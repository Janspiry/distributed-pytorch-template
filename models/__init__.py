...
def define_network(opt, network_opt):
    try:
        ...
        net = getattr(importlib.import_module(net_file_name), net_class_name)(**network_opt['args'])
        if opt['global_rank']==0:
            logger.info('Network {:s} form {:s} is created.'.format(net_class_name, net_file_name))
    except:
        raise NotImplementedError('Network {:s} not recognized.'.format(network_opt))
    
    if opt['phase'] == 'train' and opt['path']['resume_state'] is None:
        if opt['global_rank']==0:
            logger.info('Network weight initialization.')
        net.init_weights()
    else:
